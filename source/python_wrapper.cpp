...
pypolyhedron py_extrusion( const boost::python::list &coords, const double distance ){
...
pypolyhedron py_surface_of_revolution( const boost::python::list &coords, const double angle=360, const int segments=20 ){
    std::vector<double> tcoords;
    std::vector<int> tlines;
...
BOOST_PYTHON_MODULE(pyPolyCSG){
	...
	class_<pypolyhedron>("polyhedron")
	.def( "load_mesh",	               &pypolyhedron::initialize_load_from_file )
	.def( "make_sphere",               &pypolyhedron::initialize_create_sphere, make_sphere_overloads() )
	.def( "make_box",                  &pypolyhedron::initialize_create_box,    make_box_overloads() )
    .def( "make_cylinder",             &pypolyhedron::initialize_create_cylinder, make_cylinder_overloads() )
	.def( "make_cone",                 &pypolyhedron::initialize_create_cone,   make_cone_overloads() )
	.def( "make_torus",                &pypolyhedron::initialize_create_torus,  make_torus_overloads() )
    .def( "make_extrusion",            &pypolyhedron::initialize_create_extrusion, make_extrusion_overloads() )
    .def( "make_surface_of_revolution",&pypolyhedron::initialize_create_surface_of_revolution, make_surface_of_revolution_overloads() )
	.def( "translate",                 &pypolyhedron::translate )
	.def( "rotate",                    &pypolyhedron::rotate )
	.def( "scale",                     &pypolyhedron::scale )
	.def( "mult_matrix_3",             &pypolyhedron::py_mult_matrix_3 )
	.def( "mult_matrix_4",             &pypolyhedron::py_mult_matrix_4 )
	.def( "save_mesh",                 &pypolyhedron::output_store_in_file )

    .def( "num_vertices",              &pypolyhedron::num_vertices )
    .def( "num_faces",                 &pypolyhedron::num_faces )
    .def( "num_face_vertices",         &pypolyhedron::num_face_vertices )
    .def( "get_vertex",                &pypolyhedron::py_get_vertex_coordinates )
    .def( "get_face",                  &pypolyhedron::py_get_face_vertices )
    .def( "get_vertices",              &pypolyhedron::py_get_vertices )
    .def( "get_triangles",             &pypolyhedron::py_get_triangles )

	.def( self + pypolyhedron() )
	.def( self - pypolyhedron() )
	.def( self * pypolyhedron() )
